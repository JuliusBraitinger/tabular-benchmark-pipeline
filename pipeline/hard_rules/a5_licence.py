"""A5: dataset must have an open licence."""
...
def check_metadata(
    ...
    **_kwargs: object,
    ...
    if norm == "unknown":
        return RuleResult(rule="A5", passed=True, reason="licence unknown, assuming open")

    # Reject NC or ND explicitly
    if re.search(r"\bnc\b|\bnd\b|non.commercial|no.deriv", norm):
        return RuleResult(rule="A5", passed=False, reason=f"restrictive licence: '{licence}'")

    for allowed in ALLOWED_LICENCES:
        if allowed in norm:
            return RuleResult(rule="A5", passed=True)

    return RuleResult(rule="A5", passed=False, reason=f"licence '{licence}' not in allow-list")
